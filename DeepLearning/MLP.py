...
# ImplÈmentation du MLP
# Couche d'entrÈes, 1 couche cachÈe et couche de sorties
#
# Code similaire ‡ Adaline
...
class NeuralNetMLP(object):
    ...
    def fit(self, X, y, print_progress=False):
        ...
        for i in range(self.epochs):  # Nombre de passage sur le dataset
            ...
            self.eta /= (
                        1 + self.decrease_const * i)  # Permet de rÈduire le nombre d'epochs nÈcessaire ‡ la convergence en limitant les risques de "pas" trop grand!

            if print_progress:
                sys.stderr.write('\rEpoch: %d/%d' % (i + 1, self.epochs))
                sys.stderr.flush()

            if self.shuffle:  # on mÈlange le dataset ‡ chaque epoch
                idx = np.random.permutation(y_data.shape[0])
                X_data, y_enc = X_data[idx], y_enc[:, idx]

            mini = np.array_split(range(y_data.shape[0]),
                                  self.minibatches)  # Si le mode minibatch est activÈ, le dataset en entrÈe est divisÈ en batch pour le calcul des gradients
            for idx in mini:
                # feedforward
                a1, z2, a2, z3, a3 = self._feedforward(X_data[idx], self.w1,
                                                       self.w2)  # Ce que nous avons vu jusqu'‡ prÈsent
                cost = self._get_cost(y_enc=y_enc[:, idx], output=a3, w1=self.w1, w2=self.w2)
                self.cost_.append(cost)

                # compute gradient via backpropagation
                #
                # Nous verrons plus en dÈtails
